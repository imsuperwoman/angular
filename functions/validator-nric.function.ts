...
export function nricValidator(minAge: any, maxAge: any, groupControl: any): ValidationErrors | null {
  ...
  if (idTypeValue == 'NRIC') {
    ...
    if (idNoValue.length == 0) {
      ...
      if (idNoValue.length < 12) {
        ...
        let notInTheFuture = userYear.toString() == today.getFullYear() == moment().diff(moment(userDob), 'days') < 1;
        ...
        else if (userYear > todayYear - Number(minAge) || userYear < todayYear - maxAge) {
          idNoControl?.setErrors({ invalidAgeRange: true });
          dobControl?.setValue(null);
          genderControl?.setValue(null);
        }
        else if (userDob instanceof Date && !isNaN(+userDob) && notInTheFuture) {
  ...
  if (idNoControl?.getError("required")) {
    ...
    return ({ type: "invalidAgeRange", message: 'The age eligibility is ' + minAge + ' to ' + maxAge + ' years old.' });
  }
