...
@Component({
  selector: 'payment-razer-dialog',
  templateUrl: './payment-razer-dialog.component.html',
  styleUrls: ['./payment-razer-dialog.component.scss'],
})
export class PaymentRazerDialogComponent implements OnInit {
  ...
  getPaymentURL() {
    ...
    let backendURL = `${this.baseConfig.API_URL}v1/openapi/payment/status?productCode=${this.paymentData?.productCode}`;
    let responseURL = `${this.baseConfig.API_URL}v1/openapi/payment/response?productCode=${this.paymentData?.productCode}`;
  ...
  submitPayment(paymentOption: string) {
    //https://pay.merchant.razer.com/RMS/pay/Alzazo_Dev/TNG-EWALLET
    this.url = 'https://sandbox.merchant.razer.com/RMS/pay/SB_Alzazo/' + paymentOption + '/index.php',
